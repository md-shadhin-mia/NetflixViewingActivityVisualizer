...
/**
 * 
 * Download the Netflix viewing history file from the server. This requires a
 * Netflix session. We
 * could achieve this nicely by asking the user for their credentials or session
 * hijacking by extracting
 * their cookies.
 * 
 * As it's bothersome to configure selenimum drivers for such a small gain don't
 * bother.
 * Maybe later as nice assignment.
 * 
 * 
 * @author md-shadhin
 *         Instead we could directly send the correct packages to netflix
 *         instead of relying on htmlunit?
...
public class NetflixViewingActivityDownloader {
	...
	private boolean downloadUsingSelenium() {
		...
		// try {
		// WebElement response =
		// waitUntilResponse.until(ExpectedConditions.presenceOfElementLocated(By.className("profile-gate-label")));
		//
	...
	private boolean downloadUsingHTMLUnit() {
		...
		//
		// Optional<HtmlForm> oForm = forms.stream().filter(form ->
		// form.getAttribute("class").equals("login-form"))
		// .findFirst();
		//
		// //Alternative ..
		// netflixLandingPage.getDocumentElement().getElementsByAttribute("form",
		// "class", "login-form").get(0)
		// if (oForm.isPresent()) {
		//
		...
		// signinForm.getElementsByAttribute("button", "type", "submit").get(0);
		...
		// signinForm.getInputByName("password");
		//
		// //Fill out the form
		...
		// } else {
		// System.err.println(
		// "Fatal: Could not find netflix signin form. Maybe the site layout changed.
		// Please download the viewing activity file manually. Abort");
		// return false;
		// }
		//
		//
